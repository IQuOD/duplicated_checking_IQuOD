...
**This algorithm, namely as DC_OCEAN, aims at detecting the ocean *in-situ* duplicate profiles by reducing the computational intensity in a cost-effective way.**
...
This script aims at reading the metadata and other information from the orginal netCDF files (we uses the WOD18 single netCDF format) and then preprocess the metadata.

(2) ./support/N01_possible_duplicates.py

This scripts aims at utilizing 14 distinct screening criteria to calculate the 'DNA' and identify possible duplicate pairs. The output is a possible duplicate pair list file (*.txt).
...
|     3     | M01_MAIN_check_nc_duplicate_manual.py | Determine  whether the potential duplicates from N03 is the real  duplicates or not by manually check.. |
|     4     |  M02_MAIN_check_nc_duplicate_list.py  |       The same as Order 3, but in automatically check.       |

 For more deatils and interpreation of the codes above, please refer to Song et al., 2023, Frontier in Marine Science.
...
Now the 'DC_OCEAN' package is uploaded to pypi (https://pypi.org/project/DC-OCEAN/1.1). For those of you interests, you can easily and freely accessed via 'pip' with the following steps:
...
Next, use the `DNA_summary.npz` file as input for the sequential execution of `support/N01_possible_duplicates.py`. These scripts apply various 'DNA' algorithms (with 14 cretiera in total) to uncover as many potential duplicates as possible. This process generates the possible duplicate pairs list:
...
用表格列出我们所使用的WOD18的变量，变量所代表的含义，及其数据的维度、存储数据类型
